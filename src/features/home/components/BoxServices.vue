    ...
    (context) => {
      ...
      if (!isMobile) {
        tl.fromTo(wrapperEl,
          ...
          { clipPath: "inset(0% 0% 0% 0%)", duration: 0.4, ease: "none" }, 0);
      } else {
      ...
      if (!isMobile && subRefs.value.length > 0) {
        const subItems = subRefs.value.filter(r => r !== null && r !== undefined);
        ...
        <div class="box-services-list">
          <div class="box-services-list-item" v-for="(service, index) in services" :key="service.name">
            <p class="box-services-list-item-name">
              <AppearingText :text="service.name" :steps="1" :duration="0.35"
...
.box-services {
  ...
  &-title {
    font-size: var(--font-size-title-xs); font-weight: 700;
